...
def build_task_list_response(state: AgentState) -> str | None:
    ...
    lines = [build_task_list_header(payload)]
    for index, task in enumerate(tasks, start=1):
        if not isinstance(task, dict):
            continue
        lines.extend(format_task_block(index, task))
...
def build_task_list_header(payload: dict) -> str:
...
def format_task_block(index: int, task: dict) -> list[str]:
    title = first_non_empty(task.get("content"), "Untitled task")
    line_one = f"{index}. {title}"

    summary = join_parts(
        [
            labeled_value("Project", task.get("project")),
            labeled_value("Iteration", task.get("iteration")),
            labeled_value("Platform", task.get("platform")),
            labeled_value("Priority", task.get("priority")),
        ]
    )

    schedule = join_parts(
        [
            labeled_value("Assignee", task.get("assignee")),
            labeled_value("Due", task.get("end_date")),
            labeled_value("Status", humanize_due_status(task.get("due_status"))),
        ]
    )

    owners = join_parts(
        [
            labeled_value("Client", task.get("client_owner")),
            labeled_value("Server", task.get("server_owner")),
            labeled_value("QA", task.get("test_owner")),
            labeled_value("PM", task.get("product_owner")),
        ]
    )
...
def humanize_due_status(value) -> str:
    mapping = {
        "overdue": "Overdue",
        "today": "Due today",
        "upcoming": "Upcoming",
